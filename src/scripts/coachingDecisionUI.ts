...
export function careerCardHtml(path: Row, ui: DecisionUiContext, editable = true, skills: Row[] = []) {
  ...
  return `<article class="career-decision-row" data-status="${ui.escapeHtml(path.status)}" data-option-type="${ui.escapeHtml(optionType)}">
    ...
    ${skillPreview.length ? `<div class="career-skill-preview"><small>Skills this route needs</small><div>${chipsHtml(skillPreview, ui.escapeHtml)}</div>${skillMapping}</div>` : ''}</div></details>\n    <footer><span><small>Optional next step</small><strong>${ui.escapeHtml(learnerNextStep)}</strong>${path.decision_deadline ? `<em>Revisit by ${ui.escapeHtml(ui.formatDate(path.decision_deadline))} if useful</em>` : ''}${learnerOpenQuestion ? `<em>Open question: ${ui.escapeHtml(learnerOpenQuestion)}</em>` : ''}</span><span class="row-actions">${ui.viewerRole === 'student' ? '' : `<button class="table-action" type="button" data-add-career-skills="${ui.escapeHtml(path.id)}">Add suggested skills</button>`}${canManage ? `<button class="table-action" type="button" data-edit-career="${ui.escapeHtml(path.id)}">Edit</button><button class="table-action" type="button" data-delete-career="${ui.escapeHtml(path.id)}">Remove</button>` : ''}</span></footer>
  </article>`;
}
