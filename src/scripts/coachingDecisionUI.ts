...
export function careerLibraryResultsHtml(matches: CareerGuide[], selectedKey: string, escapeHtml: DecisionUiContext['escapeHtml'], limit: number, offset = 0) {
  ...
  return matches.slice(offset, offset + limit).map((guide) => {
    ...
    return `<div class="career-library-result"><button type="button" data-career-preset="${escapeHtml(guide.key)}" class="${guide.key === selectedKey ? 'is-selected' : ''}"><span class="career-result-title"><strong>${escapeHtml(guide.title)}</strong>${competition === 'Highly competitive' ? '<em class="career-competition-flag">Highly competitive</em>' : ''}</span><small>${escapeHtml(guide.careerGroup)} · ${escapeHtml(careerFamilyLabel(guide.category))}</small><p>${escapeHtml(guide.summary)}</p><div class="career-result-facts"><span><b>You may like</b>${escapeHtml(interests)}</span><span><b>Typical day</b>${escapeHtml(ordinaryWork)}</span><span><b>How people start</b>${escapeHtml(entryRoute)}</span><span><b>Useful first skills</b>${escapeHtml(starterSkills)}</span><span><b>This may fit if</b>${escapeHtml(fitSignals)}</span><span><b>Optional way to learn</b>${escapeHtml(firstTest)}</span>${evidence ? `<span><b>What you might notice</b>${escapeHtml(evidence)}</span>` : ''}</div><span class="career-result-meta"><em>${escapeHtml(outlook)}</em><em>${escapeHtml(competition)}</em><em>${escapeHtml(independence)}</em><em>${guide.regulated ? 'Check registration or licence' : 'No universal licence'}</em><em>Skills: ${escapeHtml((guide.specialistSkills ?? []).slice(0, 2).join(' · ') || 'Start with the core skills')}</em></span><span class="career-result-action" data-career-preset="${escapeHtml(guide.key)}">Read the details <b aria-hidden="true">→</b></span></button><div class="career-result-save-actions" aria-label="Keep ${escapeHtml(guide.title)} open"><span>Keep this option:</span><button class="table-action" type="button" data-choose-career="primary" data-guide-key="${escapeHtml(guide.key)}">Primary</button><button class="table-action" type="button" data-choose-career="alternative" data-guide-key="${escapeHtml(guide.key)}">Secondary</button></div><label class="career-compare-toggle"><input type="checkbox" data-compare-career="${escapeHtml(guide.key)}" aria-label="Add ${escapeHtml(guide.title)} to comparison" /><span>Add to comparison</span></label></div>`;
  }).join('');
}

export function careerGuidePreviewHtml(guide: CareerGuide, escapeHtml: DecisionUiContext['escapeHtml']) {
  const quickChoice = `<div class="career-guide-save career-guide-save-quick"><p class="career-guide-save-note"><strong>Looks worth exploring?</strong> Save it now without filling in a form. You can change or remove it later.</p><div class="career-guide-actions"><button class="primary-button" type="button" aria-label="Choose as primary career option" data-choose-career="primary" data-guide-key="${escapeHtml(guide.key)}">Choose as a primary option</button><button class="secondary-button" type="button" aria-label="Keep as secondary career option" data-choose-career="alternative" data-guide-key="${escapeHtml(guide.key)}">Keep as a secondary option</button></div></div>`;
...
export function skillCardHtml(skill: Row, evidence: Row[], careers: Row[], ui: DecisionUiContext) {
  ...
  const evidenceBlock = evidence.length
    ...
    : `<div class="skill-evidence-list skill-evidence-empty"><small>No optional note saved. You can add a work sample, link, result, or feedback later if useful.</small></div>`;
