...
const satisfactionLabels = ['Not started', 'Starting point', 'Early progress', 'Building', 'Developing', 'On track', 'Growing confidence', 'Strong', 'Very strong', 'Advanced', 'Ready to stretch'];
...
export function careerCardHtml(path: Row, ui: DecisionUiContext, editable = true, skills: Row[] = []) {
  ...
  const learnerOpenQuestion = isStudent ? '' : visibleReviewQuestion;
  return `<article class="career-decision-row" data-status="${ui.escapeHtml(path.status)}" data-option-type="${ui.escapeHtml(optionType)}">
    ...
    <details class="career-card-details"><summary><span>See what the work is like</span><small>Open guide</small></summary><div class="career-card-details-body"><div class="career-reality-grid">
    ...
    ${skillPreview.length ? `<div class="career-skill-preview"><small>Skills this route needs</small><div>${chipsHtml(skillPreview, ui.escapeHtml)}</div>${skillMapping}</div>` : ''}</div></details>\n    <footer><span><small>${isStudent ? 'Optional idea' : 'Optional next step'}</small><strong>${ui.escapeHtml(learnerNextStep)}</strong>${path.decision_deadline ? `<em>Revisit by ${ui.escapeHtml(ui.formatDate(path.decision_deadline))} if useful</em>` : ''}${learnerOpenQuestion ? `<em>Open question: ${ui.escapeHtml(learnerOpenQuestion)}</em>` : ''}</span><span class="row-actions">${ui.viewerRole === 'student' ? '' : `<button class="table-action" type="button" data-add-career-skills="${ui.escapeHtml(path.id)}">Add suggested skills</button>`}${canManage ? `<button class="table-action" type="button" data-edit-career="${ui.escapeHtml(path.id)}">Edit</button><button class="table-action" type="button" data-delete-career="${ui.escapeHtml(path.id)}">Remove</button>` : ''}</span></footer>
  </article>`;
}
...
export function skillCardHtml(skill: Row, evidence: Row[], careers: Row[], ui: DecisionUiContext) {
  ...
  const feedbackBlock = (rows: Row[], kind: 'coach' | 'student') => {
    ...
    const render = (row: Row) => {
      const author = kind === 'coach' ? (ui.profileById?.(row.coach_id)?.full_name || 'Coach') : 'Student practice note';
      ...
      return `<div class="skill-feedback"><strong>${kind === 'coach' ? 'Coach feedback' : 'Student practice note'}</strong><small class="skill-feedback-meta">${ui.escapeHtml(author)} · ${ui.escapeHtml(ui.formatDate(date))}</small>${text ? `<p>${ui.escapeHtml(text)}</p>` : ''}${kind === 'coach' && row.next_focus ? `<small>Suggested next step: ${ui.escapeHtml(row.next_focus)}</small>` : ''}</div>`;
    };
    ...
    const older = items.length > 1 ? `<details class="skill-feedback-history"><summary>View all ${kind === 'coach' ? 'coach feedback' : 'student practice notes'} (${items.length})</summary>${items.slice(1).map(render).join('')}</details>` : '';
  ...
  const feedbackEditor = ui.viewerRole === 'student'
    ? `<div class="skill-feedback-editor" data-student-reflection-editor><button class="skill-feedback-toggle" type="button" data-toggle-skill-feedback aria-expanded="false">${studentReview ? 'Update practice note' : 'Add a practice note (optional)'}</button><div data-skill-feedback-panel hidden>${inlineFeedback}</div></div>`
  ...
  const proofPreview = String(skill.success_criteria || '').trim() || 'Notice what became easier, clearer, or more reliable.';
  const actionPreview = ui.viewerRole === 'student'
    ? `<div class="skill-action-preview"><div><small>What you could try</small><p>${ui.escapeHtml(practicePreview)}</p></div><div><small>What you may notice</small><p>${ui.escapeHtml(proofPreview)}</p></div></div>`
    : `<div class="skill-action-preview"><div><small>Start here</small><p>${ui.escapeHtml(practicePreview)}</p></div><div><small>Optional progress note</small><p>${ui.escapeHtml(proofPreview)}</p></div></div>`;
  ...
  const quickScoreLabel = ui.viewerRole === 'student' ? 'Your score' : 'Coach score';
  ...
  const workGrid = ui.viewerRole === 'student'
    ? `<details class="skill-work-details"><summary><span>Practise this skill</span><small>Open practice</small></summary><div class="skill-work-grid"><span><small>What you could try</small><p>${ui.escapeHtml(skill.practice_method || 'Choose one repeated real task.')}</p></span><span><small>What you may notice</small><p>${ui.escapeHtml(skill.success_criteria || 'Notice what became easier, clearer, or more reliable.')}</p></span></div></details>`
    : `<details class="skill-work-details"><summary><span>Development plan</span><small>Open plan</small></summary><div class="skill-work-grid"><span><small>Development outcome</small><p>${ui.escapeHtml(skill.development_goal || 'Define the capability this work should improve.')}</p></span><span><small>How to practise</small><p>${ui.escapeHtml(skill.practice_method || 'Choose a repeated real task, not only a course.')}</p></span><span><small>Proof of competence</small><p>${ui.escapeHtml(skill.success_criteria || 'Define the output, standard, and feedback that will count as proof.')}</p></span></div></details>`;
  ...
  const satisfactionHtml = ui.viewerRole === 'student'
    ? `<div class="skill-satisfaction"><span><small>Your score</small><strong>${ui.escapeHtml(studentScore)}</strong></span><small class="rating-scale-inline">0 = not started · 5 = building with support · 10 = confident in a new situation</small></div>`
    : `<div class="skill-satisfaction"><span><small>Coach score</small><strong>${ui.escapeHtml(coachScore)}</strong></span><span><small>Student score</small><strong>${ui.escapeHtml(studentScore)}</strong></span><small class="rating-scale-inline">Scale: 0 not started · 1–2 starting · 3–4 building · 5–6 useful with support · 7–8 working well · 9–10 confident in a new situation</small></div>`;
...
export function skillRoadmapHtml(skills: Row[], allEvidence: Row[], careers: Row[], ui: DecisionUiContext) {
  ...
  const skillGroupNames = ui.viewerRole === 'student'
    ? { foundation: 'Useful in any career', career: 'For your career options', future: 'For changing work', other: 'Other skills', all: 'All skills' }
    : { foundation: 'Core skills', career: 'Career-specific skills', future: 'Skills for change', other: 'Other skills', all: 'All skills' };
  const tabs = `<nav class="skill-scope-tabs" role="tablist" aria-label="Skill groups"><button type="button" role="tab" aria-selected="${initialTab === 'foundation'}" data-skill-scope-tab="foundation">${tabLabel(skillGroupNames.foundation, 'foundation')}</button><button type="button" role="tab" aria-selected="${initialTab === 'career-specific'}" data-skill-scope-tab="career-specific">${tabLabel(skillGroupNames.career, 'career-specific')}</button>${futureReady ? `<button type="button" role="tab" aria-selected="false" data-skill-scope-tab="future-ready">${tabLabel(skillGroupNames.future, 'future-ready')}</button>` : ''}${other ? `<button type="button" role="tab" aria-selected="false" data-skill-scope-tab="other">${tabLabel(skillGroupNames.other, 'other')}</button>` : ''}<button type="button" role="tab" aria-selected="false" data-skill-scope-tab="all">${tabLabel(skillGroupNames.all, 'all')}</button></nav>`;
  ...
  const scaleLegend = ui.viewerRole === 'student' ? '<p class="skill-scale-legend"><strong>Rating guide:</strong> 0 = not started · 1–2 = just beginning · 3–4 = building with support · 5–6 = usable in familiar work · 7–8 = reliable in most work · 9–10 = confident in a new situation.</p><p class="skill-scope-explanation"><strong>How the groups work:</strong> Useful in any career are portable foundations. For your career options are skills linked to the options you saved. For changing work are useful as tools and expectations change. “All skills” combines every group. Ratings and notes are optional.</p>' : '<p class="skill-scale-legend"><strong>Rating guide:</strong> 0 = not started · 1–2 = just beginning · 3–4 = building with support · 5–6 = usable in familiar work · 7–8 = reliable in most work · 9–10 = confident in a new situation.</p><p class="skill-scope-explanation"><strong>How the groups work:</strong> Core skills are portable foundations, Career-specific skills are linked to your saved career options, and Skills for change are future-ready capabilities. “All skills” combines every group; changing-work skills are different from uncategorised Work skills.</p>';
