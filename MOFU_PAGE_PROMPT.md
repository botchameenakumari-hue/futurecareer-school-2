...
- Treat accuracy, the Non-Negotiable Truth Rule, public-copy tone, anti-doorway rules, the honest-comparison
  requirement, schema rules, and factual guardrails as hard rules.
- Treat positioning angles, differentiation rows, section-ingredient lists, and design direction as
  thinking tools. Use them only when they make the page a more useful, truthful, and natural answer to
  this exact comparison.
- Apply every messaging-bank angle that is genuinely relevant to the supplied keyword, adapted to that
  comparison's audience and decision pressure — not pasted in as generic filler.

---

## Research Before Writing

Before writing a MOFU page, research the real comparison behind the keyword. Do not write from a guess.

Understand:
- what the person is actually weighing (cost, format, provider type, whether a free layer is enough yet)
  and the expensive wrong turn they are trying to avoid
- how the options in the market genuinely differ, so the honest trade-off is real rather than assumed
- the real objections and doubts someone at this exact comparison keyword would raise
- realistic cost or value benchmarks for the category, so a fees/worth-it page reads as fair
- any changing fact (market claims, typical pricing context, timelines) against a current primary source

Research standards:
- collect updated, practical information, not stale assumptions or definition-level content
- external research may inform comparison nuance, objection realism, and trust context, but it must never
  be used to invent business claims about Future Career School or unsupported claims about competitors
- if a comparison point is not well supported, verify it harder or leave it out
...
- Anything you take from it must still pass the Non-Negotiable Truth Rule and stay inside the Approved
  MOFU Facts and Messaging Bank. Do not turn a handed-over quote into a new guarantee, price, or outcome
  claim, and do not publish on-site testimonials or ratings as `Review`/`AggregateRating` markup
  (see Schema Rules).
...
for a test, proof-of-work block, results timeline, or a "how long until you see value" claim. People
move at different speeds depending on their situation, energy, money, and support.
...
MOFU comparison keywords ("is career counselling worth it", "career counselling reviews",
"best career guidance service") make it tempting to reach for review markup. Do not.

- Do not use self-serving `Review` or `AggregateRating` markup for Future Career School, its services,
  plans, or on-site testimonials. Google disallows self-serving review markup for the entity that owns
  the page, and it is a manual-action risk.
- Do not invent a rating, reviewer, testimonial, aggregate score, star count, or dummy review section to
  qualify for a rich result.
- A "reviews" or "worth it" page does not qualify for `Review` schema unless it genuinely reviews one
  specific named third-party item, with the reviewed item, author, review text, and any rating all
  clearly visible on the page — and even then, never for Future Career School's own service.
- Keep `Article` (or the bridge-page `FAQPage + BreadcrumbList`) as the schema. Validate any genuine
  third-party review markup with Google's Rich Results Test before publishing.
...
used by the top-of-funnel prompt. It is included here because MOFU comparison and consideration pages
still need real, keyword-relevant substance about the person doing the comparing — their weekly reality,
the expensive wrong turn they fear, the proof formats their market respects, their hidden barriers, and
the safer adjacent paths — so the honest-comparison layer of the page is grounded rather than generic.

**How to use it on a MOFU page:**
- It describes the *reader's* world, not Future Career School's offering. Using it does not conflict with
  the Non-Negotiable Truth Rule — that rule governs claims about the business, prices, and outcomes.
- Pull only the pointers that genuinely fit this comparison's audience and decision. Skip everything else.
- Adapt every pointer to the specific keyword; never paste a pointer in as generic wording.
- These pointers support the 30% comparison-specific-uniqueness bar in the Anti-Doorway Rules.
- Keep all Future Career School positioning and CTA language inside the Approved MOFU Facts and Messaging
  Bank above. This appendix informs the honest-comparison substance, not the commercial claims.
...
## Custom Career Perspective Bank (optional — comparison depth and conversion support)
...
**On a MOFU comparison or consideration page they are optional.** A MOFU page's job is an honest
comparison that also makes Future Career School's advantage visible and moves a decided reader to an
easy next step — not a full career-philosophy lesson.

### When to reach for this bank on a MOFU page

Pull a perspective in only when it genuinely does one of these for this exact comparison keyword:
- deepens the **honest trade-off** the page is built on (e.g. on "free vs paid career guidance", the
  "information is nearly free; the paid value must come from structure, feedback, accountability, and
  a valued outcome" perspective is directly on point)
- strengthens the **value-for-cost reasoning** on a fees / "is it worth it" page with honest substance
  rather than "quality matters" filler
- reinforces an **approved differentiation angle** already in the messaging bank (e.g. "degrees have a
  place; degree-only thinking is the problem", or the right-skill-portfolio → high-income-opportunities
  → earlier-financial-freedom chain)
- adds specific, honest substance that helps meet the 30% comparison-specific-uniqueness bar

### How to use it (these rules replace the top-of-funnel rules)

- **Never mandatory.** If a perspective does not clearly improve the comparison or build trust, leave it
  out. Do not work down the list. Skipping the whole bank is fine for most pages.
- **Keep it proportionate.** A sentence or two inside a section that already exists (the trade-off, the
  value-for-cost reasoning, an objection, an FAQ). It must not overtake the comparison or turn the page
  into a blog article.
- **Subordinate to the messaging bank.** The Non-Negotiable Truth Rule and the Approved MOFU Facts and
  Messaging Bank always win. A perspective may add reader-world context; it must not add a new claim
  about Future Career School, its prices, plans, deliverables, or outcomes.
- **Keep the comparison honest.** A perspective must not be used to tilt a genuine trade-off into a
  one-sided pitch. If the honest answer is "the free layer may be enough for now", the perspective
  should not paper over that.
- **No promises.** Keep multipliers, budget shares, timelines, and rankings as heuristics, never as
  guarantees of income, financial freedom, or a fixed multiplier.
- **Adapt, never paste.** Tie the point to this comparison's audience, cost, and decision. Generic stock
  wording is worse than leaving it out.
