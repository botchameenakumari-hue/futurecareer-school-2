...
async function completeAssessment(page) {
  for (let question = 0; question < 35; question += 1) {
  ...
  throw new Error('Class 11 and 12 assessment did not reach the results page.');
}
...
async function expectPdfDownload(page, button) {
  ...
  expect(download.suggestedFilename()).toBe('class-11-12-career-assessment-report.pdf');
  expect(await download.failure()).toBeNull();
}

test('Class 11 and 12 preserves both PDF controls and exports the complete dedicated report', async ({ page }) => {
  const pageErrors = [];
  page.on('pageerror', (error) => pageErrors.push(error.message));

  const baseUrl = process.env.PLAYWRIGHT_BASE_URL || 'http://localhost:4321';
  await page.goto(`${baseUrl}/services/assessments/class-11-to-12/`);
  await completeAssessment(page);

  const results = page.locator('#results-container');
  await expect(results).toBeVisible();
  expect((await results.innerText()).length).toBeGreaterThan(30_000);
  await expect(results).toContainText('The Deep Thinker');
  await expect(results).toContainText('The 4 Core Human Skills (Forever Valuable)');
  await expect(results).toContainText('The 4 Business Types (For Later)');
  ...
  const bottomDownload = results.locator('[data-class-11-pdf-download]');
