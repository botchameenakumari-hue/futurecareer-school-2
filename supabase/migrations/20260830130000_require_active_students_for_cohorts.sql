-- Cohort placement is an operational action for active students only.
-- Keep the rule in the database as well as in the dashboard picker.
...
begin
  ...
  if target_student.account_status is distinct from 'active' then
    raise exception 'Only active students can be placed in a cohort.';
  end if;
